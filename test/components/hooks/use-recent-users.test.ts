...
describe("useRecentUsers (integration)", () => {
  ...
  it("should start with loading state, no data, and hasMore true", () => {
    // Mock fetch BEFORE calling renderHookServer so the hook uses our mock
    const mockFetch = vi.spyOn(globalThis as any, "fetch");
    mockFetch.mockImplementation(() => {
      // Return empty users array for any fetch call
      return Promise.resolve(
        new Response(JSON.stringify({ recentUsers: [] }), {
          status: 200,
          statusText: "OK",
        }),
      );
